...
import com.jxdx.classroom.databinding.ActivityGeogebraBinding
...
open class GeoGebraActivity : BaseActivity<ActivityGeogebraBinding>() {

    private var whiteBoardMode = false

    companion object {
        fun actionStart(context: Context, whiteBoardMode: Boolean) {
            context.startActivity(Intent(context, GeoGebraActivity::class.java).apply {
                putExtra("whiteBoardMode", whiteBoardMode)
            })
        }
    }

    override fun bindLayout(): ActivityGeogebraBinding {
        return ActivityGeogebraBinding.inflate(layoutInflater)
    }
    ...
    override fun initView() {
        ...
        intent.extras?.let {
            whiteBoardMode = intent.getBooleanExtra("whiteBoardMode", false)
        }
        ...
        view.apply {
            x5WebView.apply {
                ...
                loadUrl("file:///android_asset/webpage/geogebra/geogebra_graphing.html") // 默认绘图计算器
            }
            ...
            geogebraMenuButton.apply {
                ...
                setOnTouchListener { v, event ->
                    when (event.action) {
                        ...
                        MotionEvent.ACTION_UP -> {
                            if (!isDrag) {
                                Log.d("GeoGebraActivity", "Confirm clicked")
                                v.performClick()
                            }
                        }
                ...
                setOnClickListener {
                    XPopup.Builder(this@GeoGebraActivity)
                        .asCenterList(
                            "模式选择",
                            arrayOf(
                                "绘图计算器",
                            ...
                            when (position) {
                                0 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_graphing.html")
                                }

                                1 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_geometry.html")
                                }

                                2 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_3d.html")
                                }

                                3 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_classic.html")
                                }

                                4 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_suite.html")
                                }

                                5 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_evaluator.html")
                                }

                                6 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_scientific.html")
                                }

                                7 -> {
                                    x5WebView.loadUrl("file:///android_asset/webpage/geogebra/geogebra_note.html")
                                }
                            }
                        }
                        .show()
